            ...
            <section class="max-w-6xl mx-auto px-4 mb-12 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="glass-card rounded-3xl p-6 relative overflow-hidden">
                    ...
                    <div class="space-y-2 max-h-[200px] overflow-y-auto modal-scroll pr-2">
                        <div v-for="member in familyData.equity.members" :key="member.name" class="flex justify-between items-center bg-white/40 px-3 py-2.5 rounded-xl text-sm border border-white/50 shadow-sm transition">
                            ...
                            <div class="flex flex-col items-end w-5/12"><span class="font-bold text-green-600 text-sm">+{{ formatCurrencyInt((member.principal / familyData.equity.pricePerShare) * familyData.equity.dividendRate * 0.8) }} <span class="text-[9px] font-normal text-green-600 border border-green-200 bg-green-50 px-1 rounded ml-1">税后</span></span></div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="max-w-6xl mx-auto px-4 mb-16">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div v-for="loan in processedLoans" :key="loan.id" class="glass-card rounded-2xl p-5 hover:shadow-lg transition-shadow relative overflow-hidden">
                        ...
                        <div class="bg-white/50 p-4 rounded-xl border border-white space-y-3 shadow-sm">
                            <div class="flex justify-between items-center text-sm border-b border-gray-200/50 pb-2"><span class="text-gray-600 font-medium">剩余本金</span><span class="font-bold text-gray-800">{{ formatCurrency(loan.left) }}</span></div>
